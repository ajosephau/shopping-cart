...
class TestStepOne(unittest.TestCase):
    def test_main_scenario(self):
        ...
        self.assertEqual(
            shopping_cart.num_items, 5, "Incorrect number of items in cart"
        )
        self.assertDictEqual(
            shopping_cart.products, {product_dove_soap: 5}, "Cart is incorrect"
        )
        self.assertEqual(
            len(shopping_cart.products.keys()), 1, "Multiple items in cart"
        )
        self.assertEqual(
        ...
        self.assertEqual(shopping_cart.total, 199.95, "Incorrect cart total")
