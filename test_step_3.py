...
class TestStepThree(unittest.TestCase):
    ...
    def test_main_scenario(self):
        ...
        self.assertEqual(product_axe_deo.unit_price_excl_tax, 99.99, "Product price is incorrect")
        ...
        # Then:
        self.assertEqual(shopping_cart.num_items, 4, "Incorrect number of items in cart")
        self.assertEqual(len(shopping_cart.products.keys()), 2, "Incorrect number of cart products")
        self.assertDictEqual(shopping_cart.products, {product_dove_soap: 2, product_axe_deo: 2}, "Cart is incorrect")
        # The shopping cart should contain 2 Dove Soaps each with a unit price of 39.99
        self.assertEqual(shopping_cart.products[product_dove_soap], 2, "Cart has incorrect number of Dove Soap items")
        self.assertEqual(list(shopping_cart.products.keys())[0].name, "Dove Soap", "Cart item name incorrect")
        ...
        self.assertEqual(shopping_cart.products[product_axe_deo], 2, "Cart has incorrect number of Axe Deo items")
        self.assertEqual(list(shopping_cart.products.keys())[1].name, "Axe Deo", "Cart item name incorrect")
        self.assertEqual(list(shopping_cart.products.keys())[1].unit_price_excl_tax, 99.99, "Cart unit price incorrect")
